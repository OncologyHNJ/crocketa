...
## modify barcodes in both objects to match barcodes
...
if (is.null(cells) | all(!(cells %in% c("T-AB", "T-GD", "B-cells")))){
  ...
  # cell box is empty (null) or not one of the 3 possible sets of cells
    ...
    for (label in cells){ # might be more than one cell type at a time
      ...
      saveRDS(combined, paste0(dir.name, "/", folders[5], "/", "combined_", label, ".rds")) # save and check BCR/TCR overlapping afterwards
...
message("2- starting scRepertoire Analysis")
...
quiet_ggsave(paste0(dir.name, "/", folders[5], "/0.1_Clonetype_freqs_DimPlot.pdf"), plot = p0.1, scale = 1.5)
t0.1 <- table(seurat@meta.data[["cloneType"]])
write_xlsx(as.data.frame(t0.1), paste0(dir.name, "/", folders[5], "/0.1_Clonetype_abundance.xlsx"))
...
write_xlsx(totaldf, paste0(dir.name, "/", folders[5], "/1_Clonetype_by_", cond_i, ".xlsx"))
...
if(! is.null(samples_vdj_T) ){
	message("-- viral annotation for TCRs")
	...
	for (chain in c("alpha", "beta")){
		...
		quiet_ggsave(paste0(dir.name, "/", folders[5], "/7.2_TOP10_ViralClones_VDJDB-",chain,".pdf"), plot = p7.2, scale = 1.5, width = 8)
	...
	pdf(paste0(dir.name, "/", folders[5], "/7.3_UMAP_viralAnn_VDJDB.pdf"), width = 7)
	...
	quiet_ggsave(paste0(dir.name, "/", folders[5], "/7.4A_Alluvial_alphaViralClonotypes_",cond_i, ".pdf"), plot = p7.4.1, scale = 1.5, width = 8)
	...
	quiet_ggsave(paste0(dir.name, "/", folders[5], "/7.4B_Alluvial_betaViralClonotypes_", cond_i, ".pdf"), plot = p7.4.2, scale = 1.5, width = 8)
	...
	quiet_ggsave(paste0(dir.name, "/", folders[5], "/7.4C_Alluvial_pairedViralClonotypes_", cond_i, ".pdf"), plot = p7.4.3, scale = 1.5, width = 8)
	...
	write_xlsx(ann_res, paste0(dir.name, "/", folders[5], "/7.5_ViralAnn.xlsx"))
	...
	message("-- 1.1. Viral annotation completed!")
} else { 
	seurat_noViral <- NULL
	message("-- 1.1. Skipping viral annotation, not available for BCRs yet") 
	}
...
for (x.cond in cond){
  ...
  for (set in sets){
    message(paste0("1.2- Performing repertoire Analysis for ", set, " - According to ", x.cond))
    dir.create(paste0(dir.name, "/", folders[5], "/", set))
    if (set == "Full_assay"){
      seurat_i <- seurat
    } else{
      ...
      dir.create(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select))
    	...
    	if (cell_select == "BCR"){
		  ...
		  p1 <- DimPlot(seurat_sel, group.by = x.cond) + 
		    ...
		    theme(plot.title = element_blank())
		  quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/1.1_Clonetype_DimPlot_by_", x.cond, ".pdf"), plot = p1, scale = 1.5)
			p1.2 <- DimPlot(seurat_sel, group.by = x.cond, split.by = "clonotype.detected", label = F, reduction='umap') +
				ggtitle("Identified Clonotype") +
				scale_color_manual(values = color_conds(length(unique(seurat_sel@meta.data[[x.cond]])))) +
				theme(plot.title = element_text(hjust = 0.5))
		  ...
		  for (per_C in levels(seurat_sel@meta.data[[x.cond]])){
		    dir.create(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C))
		    # create the per_condition subsetting of seurat_sel
		    seurat_sel_sub <- seurat_sel[, which(x = FetchData(object = seurat_sel, vars = x.cond) == per_C)]
		    ...
		    for (i in c(1,2)){ # 1 for alpha chain; 2 for beta chain
		      ...
		      for (pair in strsplit(seurat_sel_sub$CTaa, "_")){
		        chain_i <- pair[i]
		      ...
		      write.csv(chain, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C, "/7.0-Clones_", per_C, "_", id, "chain.csv"), row.names = F)
		    }
		    ## 4 - highlightClonotypes: highlight the most-frequent clonotypes per condition
		    TOPsequence <- seurat_sel_sub@meta.data[order(seurat_sel_sub@meta.data$Frequency, decreasing = TRUE),]
		    write_xlsx(TOPsequence[,c("barcode", "Frequency", "CTaa", "CTnt", "CTgene", "cloneType")], paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C, "/2_cloneTypes_byFrequency_", x.cond, "_",per_C, ".xlsx"))
		    for (i in seq(1,length(n))){ # in case more than 1 top_n specified in n
		      ...
		      seurat_sel <- highlightClonotypes(seurat_sel, 
		                                  cloneCall= "aa", 
		                                  sequence = TOP_n)
		      p2 <- DimPlot(seurat_sel, group.by = "highlight") + 
		      theme(plot.title = element_blank()) +
		      scale_color_manual(values = rev(color_clones(n[i])), na.value="lightgrey") +
		      theme(legend.text=element_text(size=rel(0.35)))
		      quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C, "/2_highlightClonotypes_", x.cond, "_",per_C, "_TOP",n[i],".pdf"), plot = p2, scale = 1.5)
		    }
		    ## AND BARPLOT TOP100 CLONES
		    TOP_100 <- head(unique(TOPsequence[["CTaa"]]), 100)
		    if (length(TOP_100) >=1){
				  df_100 <- seurat_sel_sub@meta.data %>% filter(CTaa %in% TOP_100) %>% arrange(desc(Frequency))
				  ...
				  if (length(count_data) > 0){
					...
					pdf(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C, "/6_TOP100_clonetypes_barplot_", x.cond, "_",per_C, ".pdf"), width = 10)
					print(p6.1)
					print(p6.2)
					dev.off()
					write_xlsx(count_df100, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C, "/6_TOP100_clonetypes_barplot_", x.cond, "_",per_C, ".xlsx"))
			}
			}
		    ## 5- Clonetype repertoire deep analysis
		    # create a dataframe to store unique values per clonetype range (plotted outside loop)
		    t_i <- as.data.frame(table(seurat_sel_sub@meta.data[, c("cloneType", x.cond)]))
		    if (dim(t_i)[1] > 0){
		    df_i <- seurat_sel_sub@meta.data %>%
		  ...
		  write_xlsx(as.data.frame(final_df), paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/3.1B_occupiedscRepertoire_", x.cond, "_Unique.xlsx"))
		  ...
		  quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/3.2_ClonalHomeostasis_", x.cond, ".pdf"), plot = p10_2, scale = 1.5)
		  t10_2 <- clonalHomeostasis(combined2, 
		                    cloneCall = "aa",
		                    exportTable = T)
		  write_xlsx(as.data.frame(t10_2), paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/3.2_ClonalHomeostasis_", x.cond, ".xlsx"))
		  '
		  # 10.3 Clonal Proportion
		  p4 <- clonalProportion(combined2, 
		                  cloneCall = "aa") +
		      scale_fill_manual(values = c(color_CTs(col+1)))
		  quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/4_ClonalProportion_", x.cond, ".pdf"), plot = p4, scale = 1.5)
		  t4 <- clonalProportion(combined2, 
		                  cloneCall = "aa",
		                  exportTable = T)
		  write_xlsx(as.data.frame(t4), paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/4_ClonalProportion_", x.cond, ".xlsx"))
		  # 10.4 Clonal Overlap
		  if (length(unique(seurat_sel@meta.data[[x.cond]])) > 1){
				pdf(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.1_ClonalOverlap_", x.cond, ".pdf"), width = 8.5)
				...
				t5.1 <- clonalOverlap(combined2, 
				              ...
				              method="morisita",
				              exportTable = T)
				write_xlsx(as.data.frame(t5.1), paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.1_ClonalOverlap_morisita_", x.cond, ".xlsx"))

				# additional Venn Diagram (expected to be similar to clonal overlap raw) - max 4
				venn_list <- vector("list", n_distinct(seurat_sel@meta.data[[x.cond]]))
				names(venn_list) <- sort(unique(seurat_sel@meta.data[[x.cond]]))
				for (i in 1:n_distinct(seurat_sel@meta.data[[x.cond]])){ 
				  venn_list[[i]] <- seurat_sel[, which(x = FetchData(object = seurat_sel, vars = x.cond) == names(venn_list)[i])]@meta.data %>% filter(!is.na(CTaa)) %>% pull(CTaa)
				}
				if (length(unique(seurat_sel@meta.data[[x.cond]])) <= 4){ # - max 4 lists to draw
				  p5.2A <- ggvenn(venn_list,
				        fill_color = color_conds(n_distinct(seurat_sel@meta.data[[x.cond]])),
				        stroke_size = 0.5, set_name_size = 4) + 
				    ggtitle(paste0("Common & unique Clones according to ", x.cond)) +
				    theme(plot.title = element_text(hjust = 0.5))
				  quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2A_Venn-ClonalOverlap_", x.cond, ".pdf"), plot = p5.2A, scale = 1.5)
				}

				# plot 2 vs. 2 venn diagrams
				combination2v <- combn(names(venn_list), 2)
				colv2 <- color_conds(n_distinct(seurat_sel@meta.data[[x.cond]]))
				names(colv2) <- sort(unique(seurat_sel@meta.data[[x.cond]]))
				pdf(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2B_Venn-1vs1-ClonalOverlap_", x.cond, ".pdf"))
				...
				venn_list_t100 <- vector("list", n_distinct(seurat_sel@meta.data[[x.cond]]))
				names(venn_list_t100) <- sort(unique(seurat_sel@meta.data[[x.cond]]))
				file_Names <- list.files(path = paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond), pattern = "6_TOP100_clonetypes_barplot.*xlsx$", recursive = T, full.names = T)
				files <- lapply(file_Names, read_excel, sheet = 1)
				names(files) <- dirname(list.files(path = paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond), pattern = "6_TOP100_clonetypes_barplot.*xlsx$",recursive = T))

				for (i in 1:length(names(files))){ 
				  venn_list_t100[[i]] <- seurat_sel[, which(x = FetchData(object = seurat_sel, vars = x.cond) == names(venn_list_t100)[i])]@meta.data %>% filter(CTaa %in% files[[i]][["CTaa"]]) %>% pull(CTaa)
				}
				if (length(unique(seurat_sel@meta.data[[x.cond]])) <= 4){ # - max 4 lists to draw
				  p5.2C <- ggvenn(venn_list_t100,
				    ...
				    theme(plot.title = element_text(hjust = 0.5))
				  quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2C_Venn-Top100_ClonalOverlap_", x.cond, ".pdf"), plot = p5.2C, scale = 1.5)
				  t5.2C <- list("common_clones" = Reduce(intersect, venn_list), "common_clones_t100" = Reduce(intersect, venn_list_t100))
				  sink(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2_VennCommonClonetypes_", x.cond, ".txt"))
				  print(t5.2C)
				  sink()
				  }
				...
				common_clones <- seurat_sel@meta.data %>%
				  group_by(CTaa) %>% # grouping AA clones
				  summarise("count"=n_distinct(get(x.cond))) %>% # count condition in each group
				  filter(count == length(unique(seurat_sel@meta.data[[x.cond]]))) %>% select(CTaa) %>% # filter those groups that don't have all conditions presents
				  filter(!is.na(CTaa))
				seurat_sel <- highlightClonotypes(seurat_sel, 
				                              cloneCall= "aa", 
				                              sequence = common_clones$CTaa)
				seurat_sel$common.clones <- seurat_sel@meta.data %>% rowwise() %>%
				  mutate("is.common" = ifelse(is.na(highlight), highlight, "common_clones")) %>% pull(is.common)
				p5.3A <- DimPlot(seurat_sel, group.by = "highlight") + 
				  theme(plot.title = element_blank()) +
				  scale_color_manual(values = color_clones(length(common_clones$CTaa)), na.value="grey") +
				  theme(legend.text=element_text(size=rel(0.5)))
				p5.3B <- DimPlot(seurat_sel, group.by = "common.clones") + 
				  theme(plot.title = element_blank()) +
				  scale_color_manual(values = "red", na.value="grey") 

				quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.3A_highlightCommonClonotypes_", x.cond, ".pdf"), plot = p5.3A, scale = 1.5)
				quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.3B_highlightCommonClonotypes_", x.cond, ".pdf"), plot = p5.3B, scale = 1.5)
				# create sankey diagram with ggplot
				sankey_df <- seurat_sel@meta.data %>% filter(CTaa %in% common_clones$CTaa) %>% distinct(CTaa, get(x.cond), cloneType) %>% tibble()
				colnames(sankey_df) <- c("CTaa", x.cond, "cloneType")
				write_xlsx(sankey_df, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.4_CommoncloneTypes_by", x.cond, ".xlsx"))
				...
				quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.4A_AlluvialCommonClonotypes_", x.cond, ".pdf"), plot = p5.4, scale = 1.5, width = 8)
				...
				all_data <- do.call(rbind.data.frame, combined2) %>% select(CTaa, x.cond)
				  ...
				  level_present <- levels(seurat_sel@meta.data[[x.cond]])[levels(seurat_sel@meta.data[[x.cond]]) %in% colnames(data_wide)]
				  ...
				  pdf(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2D-upsetR_intersect.pdf"), width = 15, height = 10)
				  if (dim(common_clones)[1] > 2){ # if more than 2 clones, highlight common
				      print(upset(
				          ...
				          queries = list(
				            ...
				            # list(
				            # query = intersects,
				            # params = list(sort(unique(seurat_sel@meta.data[[x.cond]]))[1], sort(unique(seurat_sel@meta.data[[x.cond]]))[2]),
				            # active = T,
				            # query.name = "M0 vs",
				            # color = "red"
				            # )'
				  ...
				  dev.off()
				  write_xlsx(data_wide, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2D-upsetR_intersect.xlsx"))
				...
				p5.5 <- ggplot(results, aes(x = Combination, y = Num_Cells, fill = Num_Cells)) +
					...
					geom_text(aes(label = Num_Cells), 
						vjust = -0.5, 
						size = 3)
				quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2E-Intersect_NCells_per", x.cond, ".pdf"), plot = p5.5, scale = 1.5, width = 8)
				write_xlsx(results, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/5.2E-NCells_intersect.xlsx"))
				}
				message("-- Draw scatterplots")
				# Draw scatterplots zoomed/scaled
				samples_list <- sort(names(combined2))
				dir.create(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/8-scatter/"))
				for (tag in c("plot", "plot_zoomed")){
					pdf(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/8-scatter/scatter_compare-scaled_", tag, ".pdf"))
					    ...
					    if (all(i != j, i != samples_list[length(samples_list)])){
					        ...
					        lim <- max(unlist(lapply(combined2, function(x){
					            ...
					            return(z)})))
							      ...
							      write.table(scatter_table, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/8-scatter/scatter_compare-", samples_list[i], ".vs.", samples_list[j],".tsv"), row.names = F, quote=F, sep = "\t")
						    }, error = function(e) {
			 ...
			 } else{message("-- Overlap section not available for single-condition assays")}
		      ...
		      message("-- Draw Heatmaps")
					for (subset in cond[cond != x.cond]) {
							if(grepl("^0|1.[0-9]$", subset)){
								    ...
								    if(subset != x.cond){
												seurat_sel@meta.data[[x.cond]] <- as.factor(seurat_sel@meta.data[[x.cond]])
												seurat_sel@meta.data[[subset]] <- as.factor(seurat_sel@meta.data[[subset]])
												coul <- brewer.pal(n = 6, "PuRd")
												totaldf <- as.data.frame(table(seurat_sel@meta.data[[subset]], !is.na(seurat_sel$CTaa))) %>% filter(Var2 == TRUE)
												...
												for (i in seq(1,length(n))){ # in case more than 1 top_n specified in n
													...
													for (per_C in levels(seurat_sel@meta.data[[x.cond]])){
														if (sum(grepl("6_TOP100",  list.files(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C)))) > 1){
																full_ctaa <- read_excel(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/level_", per_C, "/6_TOP100_clonetypes_barplot_", x.cond, "_", per_C, ".xlsx"))
																top_ctaa <- full_ctaa %>%
																head(n[i]) %>% pull(CTaa)
																# top_ctaa$freq <- top_ctaa$Count/sum(full_ctaa)
																seurat_sub <- seurat_sel[, which(x = FetchData(object = seurat_sel, vars = x.cond) == per_C)]
														...
														write.table(TOPsums, paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/2.B-heatmap_TopClones-per", x.cond, "_by_", subset, "_TOP",n[i], ".tsv"), row.names = F, quote=F, sep = "\t")
														...
														p2 <- ggplot(TOPsums, aes(x = subset, y = condition, fill = rel_value)) +
																...
																theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
														quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/2.B-heatmap_TopClones-per", x.cond, "_by_", subset, "_TOP",n[i],"_absolute.pdf"), plot = p1, scale = 1.5)
														quiet_ggsave(paste0(dir.name, "/", folders[5], "/", set, "/", cell_select, "/", x.cond, "/2.C-heatmap_TopClones-per", x.cond, "_by_", subset, "_TOP",n[i],"_relative.pdf"), plot = p2, scale = 1.5)
...
saveRDS(seurat, file = paste0(dir.name, "/",folders[5], "/seurat_scRepertoire.rds"))
