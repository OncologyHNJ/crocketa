  ...
  for (res in resolutions){
    ...
    if(length(levels(seurat@meta.data[[full_res]]))>1){
      for (cluster in levels(seurat@meta.data[[full_res]])) {
        ...
        Idents(seurat) <- seurat@meta.data[[full_res]]
...
write.table(df_pval, file = paste0(dir.name, "/",folders[7], "/pval_table.tsv"), sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
...
for (i in 1:length(genesets)){
  ...
  ggsave(paste0(dir.name, "/", folders[7], "/", names(genesets)[i], "_featureplot.pdf"), plot = p1, scale = 1.5)
  ...
  #ggsave(paste0(dir.name, "/", folders[7], "/", names(genesets)[i], "_clustree_mean.pdf"), plot = p2, scale = 1)
  ...
  #ggsave(paste0(dir.name, "/", folders[7], "/", names(genesets)[i], "_clustree_pval.pdf"), plot = p3, scale = 1) 
  
  #Plot combination
  p4 <- p2 + p3
  ggsave(paste0(dir.name, "/", folders[7], "/", names(genesets)[i], "_clustree_mean_pval.pdf"), plot = p4, scale = 1, width = 12, height=8, units = "in")
  
}
message("6. Clustree plots were produced.")

#9.9 Save seurat object
saveRDS(seurat, file = paste0(dir.name, "/",folders[7], "/seurat_complete.rds"))
