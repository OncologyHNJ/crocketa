...
if (length(interest_feat) > 1){
	pdf(paste0(dir.name, "/", folders[4], "/1.1_Expression_check_interest.pdf"))
...
if (is.null(gmx_dir) == F){
    ...
    names(files) <- gsub(".gmx", "", list.files(path = gmx_dir, pattern = sign_ext,
    ...
    for (i in seq(1,length(file_Names))) {
      ...
      pdf(paste0(dir.name, "/", folders[4], "/1.2_", file_i_name, "_Marker_gene_expression.pdf"), height = 15)  
        ...
        if (length(gene_sign) > 0){
          if (length(gene_sign) == 1){
            ...
            seurat <- AddModuleScore(seurat, 
                                      ...
                                      name = paste0("AddModuleScore_", colnames(df_perFile)[j]), seed = random_seed, nbin = score_ctrl)
   ...
   message("check addModuleScores")
    pdf(paste0(dir.name, "/", folders[4], "/1.3_", file_i_name, "_AddModuleScore.pdf"))
    print(VlnPlot(seurat, colnames(seurat@meta.data)[grepl("AddModuleScore_", colnames(seurat@meta.data))], stack = T, flip = T) +
              ggtitle(colnames(df_perFile)[j]) + NoLegend())
    ...
    pdf(paste0(dir.name, "/", folders[4], "/1.3_", file_i_name, "_featurePlots.pdf"))
...
if (is.null(tissue_scType)){
  ...
  pdf(paste0(dir.name, "/", folders[4], "/2.0_Inferred_tissue_scType.pdf"), paper = "a4r")
...
for (x in c("non_restrict", "restrict")){ # analyze both options
	...
	if(is.null(gs_i) == FALSE){ # if not null, then restriction is applied and both are analysed (otherwise only gs_list is checked)
		...
		dir.create(paste0(dir.name, "/", folders[4], "/2.1_scType_annotation_perCluster/"))
		pdf(paste0(dir.name, "/", folders[4], "/2.1_scType_Barplot_annotation_perCluster_", x,".pdf"), paper = "a4r", width = 20)
		...
		write.xlsx(table_ci, paste0(dir.name, "/", folders[4], "/2.1_scType_annotation_perCluster/2.1_scType_annotation_Cluster_",pop,"_",x,".xlsx"), row.names = TRUE)
		}
		dev.off()
		})
		...
		if (length(colnames(seurat)) < 100000){
			for (i in c(1:length(gs_i))){ # pos & neg
			    for (j in c(1:length(gs_i$gs_positive))){
	    			...
	    			seed = random_seed, nbin = score_ctrl)
	    			
	    			}
			    }
			}
		
			pdf(paste0(dir.name, "/", folders[4], "/2.2_scType_AddModuleScore.pdf"), width = 8)
			for (module in 
		...
		ggsave(paste0(dir.name, "/", folders[4], "/2.3_scType_annotation_",x,".pdf"), plot = p1, scale = 1.5)
		...
		ggsave(paste0(dir.name, "/", folders[4], "/2.3_scType_annotation_labeled_",x,".pdf"), plot = p2, scale = 1.5)
		write.xlsx(cL_results, paste0(dir.name, "/", folders[4], "/2.3_scType_annotation_scores_perCluster_",x,".xlsx"), row.names = TRUE)
		...
		write.table(ann_df_perCluster, 
				        file = paste0(dir.name, "/", folders[4], "/2.3_scType_annotation_",x,".tsv"), 
				        sep = "\t", 
				        quote = FALSE, 
				        row.names = FALSE)
		...
		if (!(length(unique(nodes[["cluster"]]))>length(ccolss))){
		    ...
		    pdf(paste0(dir.name, "/", folders[4], "/2.4_scType_annotation_summary_",x,".pdf"), width = 15)
		...
		pdf(paste0(dir.name, "/", folders[4], "/2.5_scType-perCellType_perOrig_plot.pdf"), width = 10)
  ...
  if (is.null(label_singleR)){
    ...
    for (colName in c(cond, "seurat_clusters")){
		for (i in c(1:length(ref_function))){
		  ...
		  dir.create(paste0(dir.name, "/", folders[4], "/3-", ref_DS))
		  ...
		  # saveRDS(pred.grun, paste0(dir.name, "/",folders[4], "/3.0_SingleR_predict_", ref_DS,"_", colName, ".rds"))
		  message("3.2. singleR result is saved")
		  # table(pred.grun$labels)
		  ## Annotation diagnostics
		  pdf(paste0(dir.name, "/", folders[4], "/3-", ref_DS, "/3.1_SingleR_Annotation_diagnostics_",ref_DS, ".pdf"), width = 15)
		  ...
		  p3 <- DimPlot(seurat, group.by = ref_DS, label = T, reduction='umap') +
		          theme(legend.text=element_text(size=rel(0.5)))
		  ggsave(paste0(dir.name, "/", folders[4], "/3-", ref_DS, "/3.2_SingleR_annotation_",ref_DS, ".pdf"), plot = p3, width = 7.5, scale = 1.5)

		  ## CREATE BARPLOT TO QUANTIFY ABUNDANCE OF CELL TYPES PER CLUSTER
		  # useful to define very clearly annotated clusters
		  pdf(paste0(dir.name, "/", folders[4],  "/3-", ref_DS, "/3.3_SingleR_Barplot_annotation_",ref_DS, ".pdf"), paper = "a4r", width = 20)
		  ...
		  write.xlsx(cL_results_singleR, paste0(dir.name, "/", folders[4],  "/3-", ref_DS, "/3.4_SingleR_annotation_SummPerCluster_",ref_DS, ".xlsx"), row.names = TRUE)
		  ## and select top cluster
		  ...
		  singleR_scores$type[as.numeric(as.character(singleR_scores$scores)) < singleR_scores$ncells/4] = "Unclassified"
		  ...
		  p3.5 <- DimPlot(seurat, reduction = "umap", label = TRUE, repel = TRUE, group.by = paste0(ref_DS, "_ClusterSum_", colName))  +
        		theme(legend.text=element_text(size=rel(0.5)))     
		  ggsave(paste0(dir.name, "/", folders[4], "/3-", ref_DS, "/3.4_SingleR_annotation_SummPerCluster_", ref_DS, ".pdf"), plot = p3.5, , width = 7.5, scale = 1.5)
...
if (sum(grepl("9.4.1_Azimuth_annotation.tsv", list.files(path = paste0(dir.name, "/", folders[4])))) > 0){
	# if Azimuth annotation is present
    message("5. Integrating Azimuth Analysis")
	azimuth_annotation <- read.table(paste0(dir.name, "/", folders[4], "/4.1_Azimuth_annotation.tsv"), header = T, quote = "", sep = "\t")
...
saveRDS(seurat, file = paste0(dir.name, "/",folders[4], "/seurat_annotated.rds"))
