...
if (regress_merge_effect){
  ...
  message("regressing merge effect")
} else{
  merge_var = NULL
}
if (length(colnames(seurat)) > 150000){
  	PCs <- 25 # reduce total PCs
  	seurat@assays$RNA@counts <- as(seurat@assays$RNA@counts, "dgCMatrix")
  	message("matrix sparsed for RAM optimization") # transform matrix to sparsedMatrix, seurat already works with it but it is transformed internally in these steps, so it reduces computational time
  	}
# 5. Normalization.
...
if (normalization == "standard") {
  ...
  if (is.numeric(vf)) {
    ...
    scaling_feats <- VariableFeatures(object = seurat)
    ...
    message("using all available genes")
    ...
    p1 <- VariableFeaturePlot(seurat) + theme(legend.position = "bottom")
    p1 <- p1 + LabelPoints(plot = p1, points = top10, repel = TRUE) + theme(legend.position = "bottom")
    ggsave(paste0(dir.name, "/", folders[1], "/6_variable_features_n2000.pdf"), plot = p1, scale = 1.5)

    scaling_feats <- rownames(seurat)
  }
  
  # Scaling to perform PCA.
  # Merged object check.
  if (length(levels(seurat[[merge_var]])) > 1) {
	message("regressing merge effect...")
	seurat <- ScaleData(seurat, features = scaling_feats, vars.to.regress = merge_var)
		  }
  else{ 
	seurat <- ScaleData(seurat, features = scaling_feats)
		  }
  ...
  if(regress_out == TRUE){
    if (regress_cell_cycle) {
      if (regress_merge_effect){
        seurat <- ScaleData(seurat, features = scaling_feats, vars.to.regress = c(vars_to_regress, "S.Score", "G2M.Score", merge_var))
      } else {
        seurat <- ScaleData(seurat, features = scaling_feats, vars.to.regress = c(vars_to_regress, "S.Score", "G2M.Score"))
      }
    } else {
      if (regress_merge_effect){
        seurat <- ScaleData(seurat, features = scaling_feats, vars.to.regress = c(vars_to_regress, merge_var))
      } else {
        seurat <- ScaleData(seurat, features = scaling_feats, vars.to.regress = c(vars_to_regress))
      }      
    }
  }
  ...
  if (is.numeric(vf)){ 
    seurat <- FindVariableFeatures(seurat, selection.method = "vst", nfeatures = vf)
    scaling_feats <- VariableFeatures(object = seurat)
  } else{scaling_feats <- rownames(seurat)}
  # PCA previous to cell cycle scoring.
...
seurat <- RunPCA(seurat, features = scaling_feats, npcs = PCs) 
...
p4 <- ElbowPlot(seurat, ndims = PCs) + theme(legend.position="bottom")
