...
require("devtools")
if (!require("VISION", character.only = TRUE)){
    message("Externally installing...")
    install_github("YosefLab/VISION@v3.0.2")}
...
if (seurat@active.assay != "integrated"){
  ...
  if (seurat@active.assay == "SCT") {
    ...
    meta_vis <- seurat@meta.data[,meta_columns]
    numericMeta <- vapply(seq_len(ncol(meta_vis)),
                          function(i) is.numeric(meta_vis[[i]]),
                          FUN.VALUE = TRUE)
    numericMeta <- meta_vis[, numericMeta, drop = F]
...
p1 <- ggplot() + aes(x=projections[, 1], y=projections[, 2], color=vis@metaData[,"VISION_Clusters"]) + geom_point(alpha=0.5) + xlab("UMAP_1") + ylab("UMAP_2") + ggtitle("Clusters representation UMAP") + labs(color='clusters') + scale_color_manual(values = getPalette(nlevels(vis@metaData[,"VISION_Clusters"])))
suppressMessages(ggsave(paste0(dir.name, "/", folders[9], "/CLUSTERS_REPRESENTATION_UMAP_set1.pdf"), plot = p1))
...
if (seurat@active.assay == "integrated"){
  ...
  suppressMessages(ggsave(paste0(dir.name, "/", folders[9], "/INTEGRATION_REPRESENTATION_UMAP_set1.pdf"), plot = p2))
}

# 11.4.4. Clusters vs Molecular Signatures statistics values. 
for (i in 1:nlevels(vis@metaData[,"VISION_Clusters"])){
  write.table(vis@ClusterComparisons$Signatures[["VISION_Clusters"]][[i]], file = paste0(dir.name, "/", folders[9], "/vision_table_res_", selected_res, "_cluster_", (i-1), ".tsv"), col.names = NA, quote = FALSE, sep = "\t")
}
...
write.table(stats_DF, file = paste0(dir.name, "/", folders[9], "/vision_param_values_per_geneset.txt"), col.names = NA, quote = FALSE, sep = "\t" )
...
for (genesig in colnames(vis@SigScores)){
  if (stats_DF[genesig,"FDR"] < 0.05) {
    ...
    suppressMessages(ggsave(paste0(dir.name, "/", folders[9], "/", genesig, "_UMAP.pdf"), plot = p3))
  }
}
message("6. Vision plots from signatures were produced.")

# 11.5. Save Vision Object.
saveRDS(vis, file = paste0(dir.name, "/", folders[9], "/vision_object.rds"))
