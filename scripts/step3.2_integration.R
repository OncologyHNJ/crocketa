...
path_to_seurat_object <- function(x, regress_out, vars_to_regress, regress_cell_cycle, norm_type, add_velocity) {
    ...
    if(norm_type == "standard"){
       ...
       if (is.numeric(vf)){
      seurat_obj <- RunPCA(seurat_obj, features = VariableFeatures(object = seurat_obj, nfeatures=vf), npcs = 50) 
	  } else {
		seurat_obj <- RunPCA(seurat_obj, features = rownames(seurat_obj), npcs = 50)  
	}
...
if (is.numeric(vf)){
    seurat.integrated <- RunPCA(seurat.integrated, features = VariableFeatures(object = seurat.integrated, nfeatures=vf), npcs = 50) 
} else {
    seurat.integrated <- RunPCA(seurat.integrated, features = rownames(seurat.integrated), npcs = 50)  
  }
