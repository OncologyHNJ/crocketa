...
ggsave(paste0(dir.name, "/", folders[4], "/4.1_Azimuth_annotation_perCell.pdf"), plot = p1, scale = 1.5)

azm.df <- seurat@meta.data %>% select(predicted.celltype.l2) %>% rownames_to_column("row")
write.table(azm.df, file =  paste0(dir.name, "/", folders[4], "/4.1_Azimuth_annotation.tsv"), sep = "\t", quote = F, row.names = F, col.names = T)

#  Barplots
cols <- c(cond, "seurat_clusters")
for (colName in cols){
  pdf(paste0(dir.name, "/", folders[4], "/4.2_Azimuth_Barplot_annotation_perCluster_", colName, ".pdf"), paper = "a4r", width = 20)
...
pdf(paste0(dir.name, "/", folders[4], "/4.2_Azimuth_Barplot_AbsoluteNumbers.pdf"), paper = "a4r", width = 20)
print(barplot(sort(table_ci / sum(table_ci) * 100, decreasing = TRUE), cex.names = 0.3, space = 15, 
