...
lapply(selected_cond, function(cond){
  ...
  if (length(unique(seurat@meta.data[[cond]])) >1){
    ...
    dir.create(paste0(dir.name, "/", folders[6], "/", cond))
    ...
    if (seurat@active.assay == "integrated") {
      ...
      if(length(unique(comparisons$assay_cond))>2){
        for (i in 1:length(unique(Idents(seurat)))){
          ...
          write.table(clusterX.markers, file = paste0(dir.name, "/", folders[6], "/", cond, "/cluster", unique(Idents(seurat))[i],".markers.tsv"), sep = "\t",  row.names=F, quote = FALSE)
        }
      } else {
      ...
      if (ranking){
        for (i in 1:length(unique(Idents(seurat)))){
          ...
          saveWorkbook(wb, paste0(dir.name, "/", folders[6], "/", cond, "/cluster", unique(Idents(seurat))[i],".DE.xlsx"), overwrite = TRUE)    
      ...
      ggsave(paste0(dir.name, "/", folders[6], "/", cond, "/1.1_heatmap_top5markers.pdf"), plot = p1, scale = 2)
      write.table(groupedby.clusters.markers, file = paste0(dir.name, "/", folders[6], "/", cond, "/1.1_heatmap_top5markers.tsv"), sep = "\t",  row.names=F, quote = FALSE)
      message("marker heatmap saved")
      pdf(paste0(dir.name, "/", folders[6], "/", cond, "/1.2_FeaturePlot_top5markers.pdf"))
    ...
    # 8.4. Save RDS
    saveRDS(seurat, file = paste0(dir.name, "/",folders[6], "/seurat_degs.rds"))
    ...
    if(assay_type == "integrated"){
    	message("The specified condition has only one level.")  
      	if(!file.exists( paste0(dir.name, "/", folders[6], "/seurat_degs.rds"))){
	      saveRDS(seurat, file = paste0(dir.name, "/", folders[6], "/seurat_degs.rds"))
      	}
    } else {
